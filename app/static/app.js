...
const ICONS = {
  ...
  server: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="20" height="8" x="2" y="2" rx="2" ry="2"/><rect width="20" height="8" x="2" y="14" rx="2" ry="2"/><line x1="6" x2="6.01" y1="6" y2="6"/><line x1="6" x2="6.01" y1="18" y2="18"/></svg>`,
  ...
  external: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>`,
  ...
  key: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m21 2-2 2m-1.5 1.5L14 9l-3 3-2-2-4 4 3 3 1.5-1.5L13 14l1.5-1.5L21 6V2h-4z"/></svg>`,
  plus: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" x2="12" y1="5" y2="19"/><line x1="5" x2="19" y1="12" y2="12"/></svg>`,
  ...
  wifi: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12.55a11 11 0 0 1 14.08 0"/><path d="M1.42 9a16 16 0 0 1 21.16 0"/><path d="M8.53 16.11a6 6 0 0 1 6.95 0"/><line x1="12" x2="12.01" y1="20" y2="20"/></svg>`,
  calendar: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>`,
  sparkles: `<svg class="icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/></svg>`,
        ...
        <div class="account-main-info">
          ...
          <div class="account-name-block">
            ...
            <div class="account-phone">
              <span>${maskPhone}</span> &nbsp; ${sessionBadge || boundBadge}
            </div>
          </div>
        </div>
      ...
      <div class="card-actions">
        ${acc.sessionExpired ? `
          <button class="btn btn-danger btn-launch-full" onclick="editAccount('${acc.id}')" style="margin-bottom:6px;">
            <span style="display:flex; align-items:center; gap:8px;">${ICONS.key} 重新验证登录</span>
...
async function refreshModalCaptcha() {
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      const chId = document.getElementById("acc-challenge-id");
      const chCode = document.getElementById("acc-challenge-code");
      if (chId) chId.value = data.challengeId;
      if (chCode) chCode.value = data.challengeCode;
...
function openAddAccountModal() {
  ...
  const capInput = document.getElementById("acc-captcha-code");
...
function editAccount(accId) {
  ...
  if (!acc) return;
  const titleEl = document.getElementById("modal-account-title-text") || document.getElementById("modal-account-title");
  if (titleEl) titleEl.innerText = acc.sessionExpired ? "⚠️ 重新验证天翼云账号" : "编辑天翼云账号";
...
async function saveAccount() {
  ...
  const captchaCode = document.getElementById("acc-captcha-code") ? document.getElementById("acc-captcha-code").value.trim() : "";
  const challengeId = document.getElementById("acc-challenge-id") ? document.getElementById("acc-challenge-id").value.trim() : "";
  const challengeCode = document.getElementById("acc-challenge-code") ? document.getElementById("acc-challenge-code").value.trim() : "";
...
function toggleSubCronInputs(enabled) {
  ...
  inputs.forEach(input => {
    ...
    input.style.background = enabled ? "var(--bg-input, #ffffff)" : "var(--bg-hover, #f1f5f9)";
  });
}
